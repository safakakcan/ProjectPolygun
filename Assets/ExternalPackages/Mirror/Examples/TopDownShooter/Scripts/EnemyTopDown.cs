...
namespace Mirror.Examples.TopDownShooter
{
    public class EnemyTopDown : NetworkBehaviour
    {
...
#if !UNITY_SERVER
            if (isClient) InvokeRepeating("SetSprite", 0.1f, 0.1f);
#endif
        }
        ...
        private void FindClosestTarget()
        {
            ...
            foreach (var target in PlayerTopDown.playerList)
            {
                var distanceToTarget = Vector3.Distance(transform.position, target.transform.position);
                if (target.flashLightStatus)
                ...
                if (target.playerStatus == 0 && distanceToTarget < closestDistance && distanceToTarget <= followDistance)
                {
                    ...
                    var distanceKill = Vector3.Distance(transform.position, target.transform.position);
                    if (distanceKill < distanceToKillAt) target.Kill();
                }
            }
        ...
        {
            if (closestTarget != null) agent.SetDestination(closestTarget.position);
        }
        ...
        public void Kill()
        {
            ...
            if (isServerOnly) StartCoroutine(KillCoroutine());
        }
