...
namespace Mirror.Examples.EdgegapLobby
{
    public class UILobbyList : MonoBehaviour
    {
        ...
        public Text Error;
        private readonly List<UILobbyEntry> _entries = new();

        private EdgegapLobbyKcpTransport _transport => (EdgegapLobbyKcpTransport)NetworkManager.singleton.transport;

        private void Awake()
        {
            SearchInput.onValueChanged.AddListener(arg0 => { SetLobbies(_transport.Api.Lobbies); });
        ...
        public void SetLobbies(LobbyBrief[] lobbies)
        {
            ...
            for (var i = _entries.Count; i < lobbies.Length; i++)
            {
            ...
            for (var i = lobbies.Length; i < _entries.Count; i++) _entries[i].gameObject.SetActive(false);
        }
    }
}