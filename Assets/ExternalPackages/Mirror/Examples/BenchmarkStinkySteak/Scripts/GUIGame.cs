...
namespace StinkySteak.MirrorBenchmark
{
    public class GUIGame : BaseGUIGame
    {
        ...
        protected override void Initialize()
        {
            ...
            RegisterPrefabs(new[] { _test_1, _test_2, _test_3 });
        }

        private void RegisterPrefabs(StressTestEssential[] stressTestEssential)
        {
            for (var i = 0; i < stressTestEssential.Length; i++) _networkManager.spawnPrefabs.Add(stressTestEssential[i].Prefab);
        }
        ...
        {
            for (var i = 0; i < stressTest.SpawnCount; i++)
            {
                var go = Instantiate(stressTest.Prefab);
