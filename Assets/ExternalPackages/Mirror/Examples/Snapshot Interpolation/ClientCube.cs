...
namespace Mirror.Examples.SnapshotInterpolationDemo
{
    public class ClientCube : MonoBehaviour
    {
        ...
        // <servertime, snaps>
        public SortedList<double, Snapshot3D> snapshots = new();
        ...
        private void Update()
        {
            ...
            if (snapshots.Count > 0)
            {
                ...
                if (interpolate)
                {
                    ...
                    // interpolate & apply
                    var computed = Snapshot3D.Interpolate(fromSnapshot, toSnapshot, t);
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button((lowFpsMode ? "Disable" : "Enable") + " 1 FPS")) lowFpsMode = !lowFpsMode;

            GUILayout.Label("|");

            if (GUILayout.Button("Timeline 10s behind")) localTimeline -= 10.0;
            if (GUILayout.Button("Timeline 1s behind")) localTimeline -= 1.0;
            if (GUILayout.Button("Timeline 0.1s behind")) localTimeline -= 0.1;

            GUILayout.Label("|");

            if (GUILayout.Button("Timeline 0.1s ahead")) localTimeline += 0.1;
            if (GUILayout.Button("Timeline 1s ahead")) localTimeline += 1.0;
            if (GUILayout.Button("Timeline 10s ahead")) localTimeline += 10.0;
