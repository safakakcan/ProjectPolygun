...
namespace Mirror
{
    ...
    public class HexSpatialHash2DInterestManagement : InterestManagement
    {
        ...
        private const int MAX_Q = 19; // Covers -9 to 9 (~18 km)
        private const int MAX_R = 23; // Covers -11 to 11 (~18 km)
        private const ushort MAX_AREA = 9000; // Maximum area in meters
        ...
        private readonly Stack<HashSet<NetworkIdentity>> cellPool = new();

        // Sparse array mapping cell indices to sets of NetworkIdentities
        private readonly List<HashSet<NetworkIdentity>> cells = new();

        // Maps each connection to the set of NetworkIdentities it can observe, precomputed for rebuilds
        private readonly Dictionary<NetworkConnectionToClient, HashSet<NetworkIdentity>> connectionObservers = new();

        // Reusable list for safe iteration over NetworkIdentities, avoiding ToList() allocations
        private readonly List<NetworkIdentity> identityKeys = new();

        // Tracks the last known cell position and world position of each player's connection (observer)
        private readonly Dictionary<NetworkConnectionToClient, (Cell2D cell, Vector2 worldPos)> lastConnectionPositions = new();

        // Tracks the last known cell position and world position of each NetworkIdentity
        private readonly Dictionary<NetworkIdentity, (Cell2D cell, Vector2 worldPos)> lastIdentityPositions = new();
        ...
        private readonly HashSet<NetworkIdentity> staticObjects = new();
        ...
        private void LateUpdate()
        {
            if (NetworkTime.time - lastRebuildTime >= rebuildInterval)
            {
                ...
                foreach (var conn in NetworkServer.connections.Values)
                    ...
                    {
                        var position = ProjectToGrid(conn.identity.transform.position);
                        // Only update if the position has changed significantly
                        if (!lastConnectionPositions.TryGetValue(conn, out var last) ||
                            Vector2.Distance(position, last.worldPos) >= minMoveDistance)
                        {
                            var cell = grid.WorldToCell(position); // Convert world position to grid cell
                ...
                var updateStatic = rebuildCounter >= staticRebuildInterval;
                foreach (var identity in identityKeys)
                ...
                foreach (var conn in NetworkServer.connections.Values)
                {
                    if (conn?.identity == null || !lastConnectionPositions.TryGetValue(conn, out var connPos))
                        continue;
                    ...
                    var observers = new HashSet<NetworkIdentity>();
                    ...
                    {
                        var index = GetCellIndex(neighborCells[i]);
                        if (index >= 0 && index < cells.Count && cells[index] != null)
                            foreach (var identity in cells[index])
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                var playerPosition = NetworkClient.localPlayer.transform.position;

                // Convert to grid cell using the full Vector3 for proper plane projection
                var projectedPos = ProjectToGrid(playerPosition);
                var playerCell = grid.WorldToCell(projectedPos);
                ...
                for (var i = 0; i < neighborCells.Length; i++)
                {
                    ...
                    var worldPos2D = grid.CellToWorld(neighborCells[i]);

                    // Convert to 3D position based on checkMethod
                    var worldPos = checkMethod == CheckMethod.XZ_FOR_3D
        ...
        public override void OnSpawned(NetworkIdentity identity)
        {
            ...
            var renderers = identity.gameObject.GetComponentsInChildren<Renderer>();
        ...
        private void UpdateIdentityPosition(NetworkIdentity identity)
        {
            ...
            var position = ProjectToGrid(identity.transform.position);

            // Convert position to grid cell coordinates
            var newCell = grid.WorldToCell(position);
            ...
            if (lastIdentityPositions.TryGetValue(identity, out var previous))
            {
                ...
                if (Vector2.Distance(position, previous.worldPos) >= minMoveDistance || !newCell.Equals(previous.cell))
                {
                    if (!newCell.Equals(previous.cell))
                    {
                        ...
                        var oldIndex = GetCellIndex(previous.cell);
        ...
        private void AddToCell(Cell2D cell, NetworkIdentity identity)
        {
            ...
            if (cells[index] == null) cells[index] = cellPool.Count > 0 ? cellPool.Pop() : new HashSet<NetworkIdentity>();
        ...
        public override bool OnCheckObserver(NetworkIdentity identity, NetworkConnectionToClient newObserver)
        {
            ...
            if (!lastIdentityPositions.TryGetValue(identity, out var identityPos) ||
                !lastConnectionPositions.TryGetValue(newObserver, out var observerPos))
        ...
        public override void OnRebuildObservers(NetworkIdentity identity, HashSet<NetworkConnectionToClient> newObservers)
        {
            ...
            foreach (var conn in NetworkServer.connections.Values)
            {
                ...
                if (connectionObservers.TryGetValue(conn, out var observers) && observers.Contains(identity))
        ...
        public override void OnDestroyed(NetworkIdentity identity)
        {
            ...
            if (lastIdentityPositions.TryGetValue(identity, out var pos))
            {
                var index = GetCellIndex(pos.cell);
                if (index >= 0 && index < cells.Count && cells[index] != null)
                {
        ...
        {
            var qOffset = cell.q + MAX_Q / 2; // Shift -9 to 9 -> 0 to 18
            var rOffset = cell.r + MAX_R / 2; // Shift -11 to 11 -> 0 to 22
