...
namespace Mirror
{
    ...
    public class HexSpatialHash3DInterestManagement : InterestManagement
    {
        ...
        private const int LAYER_OFFSET = 18; // Offset for -18 to 17 layers
        private const int MAX_LAYERS = 36; // Total layers for ±9 km (18 km)
        ...
        private readonly Dictionary<NetworkConnectionToClient, (Cell3D cell, Vector3 worldPos)> lastConnectionPositions = new();

        // Tracks the last known cell position and world position of each NetworkIdentity for efficient updates
        private readonly Dictionary<NetworkIdentity, (Cell3D cell, Vector3 worldPos)> lastIdentityPositions = new();
        ...
        private void Awake()
        {
            ...
            var maxSize = MAX_Q * MAX_R * MAX_LAYERS;
        ...
        private void OnDrawGizmos()
        {
            ...
            if (NetworkClient.localPlayer != null)
            {
                ...
                for (var i = 0; i < neighborCells.Length; i++)
                {
                    ...
                    var worldPos = grid.CellToWorld(neighborCells[i]);

                    // Determine the layer relative to the player's cell for color coding
                    var relativeLayer = neighborCells[i].layer - playerCell.layer;
        ...
        private int GetCellIndex(Cell3D cell)
        {
            ...
            var layerOffset = cell.layer + LAYER_OFFSET; // Shift -18 to 17 -> 0 to 35
