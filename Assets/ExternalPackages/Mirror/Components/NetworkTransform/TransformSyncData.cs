...
namespace Mirror
{
    ...
    public static class SyncDataReaderWriter
    {
        public static void WriteSyncData(this NetworkWriter writer, SyncData syncData)
        {
            ...
            // Write position
            if ((syncData.changedDataByte & Changed.PosX) > 0) writer.WriteFloat(syncData.position.x);

            if ((syncData.changedDataByte & Changed.PosY) > 0) writer.WriteFloat(syncData.position.y);

            if ((syncData.changedDataByte & Changed.PosZ) > 0) writer.WriteFloat(syncData.position.z);

            // Write rotation
            if ((syncData.changedDataByte & Changed.CompressRot) > 0)
            {
                if ((syncData.changedDataByte & Changed.Rot) > 0) writer.WriteUInt(Compression.CompressQuaternion(syncData.quatRotation));
            }
            else
            {
                if ((syncData.changedDataByte & Changed.RotX) > 0) writer.WriteFloat(syncData.quatRotation.eulerAngles.x);

                if ((syncData.changedDataByte & Changed.RotY) > 0) writer.WriteFloat(syncData.quatRotation.eulerAngles.y);

                if ((syncData.changedDataByte & Changed.RotZ) > 0) writer.WriteFloat(syncData.quatRotation.eulerAngles.z);
            }

            // Write scale
            if ((syncData.changedDataByte & Changed.Scale) > 0) writer.WriteVector3(syncData.scale);
        }
