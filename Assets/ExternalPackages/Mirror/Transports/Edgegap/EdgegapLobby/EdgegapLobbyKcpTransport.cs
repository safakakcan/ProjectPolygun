...
namespace Edgegap
{
    ...
    public class EdgegapLobbyKcpTransport : EdgegapKcpTransport
    {
        ...
        [Header("Lobby Settings")] [Tooltip("URL to the Edgegap lobby service, automatically filled in after completing the creation process via button below (or enter manually)")]
        ...
        public TransportStatus Status
        {
            get
            {
                if (!NetworkClient.active && !NetworkServer.active) return TransportStatus.Offline;
            ...
            Api.CreateLobby(_request.Value, lobby =>
                {
                    ...
                    Api.StartLobby(new LobbyIdRequest(_lobbyId), () => { StartCoroutine(WaitForLobbyRelay(_lobbyId, true)); }, error =>
                    {
                        _status = TransportStatus.Error;
                        var errorMsg = $"Could not start lobby: {error}";
                ...
                {
                    _status = TransportStatus.Error;
                    var errorMsg = $"Couldn't create lobby: {error}";
                    Debug.LogError(errorMsg);
                    OnServerError?.Invoke(0, TransportError.Unexpected, errorMsg);
                });
        }
        ...
        public override void ServerStop()
        {
            ...
            {
                // yay
            }, error => { OnServerError?.Invoke(0, TransportError.Unexpected, $"Failed to delete lobby: {error}"); });
        }
        ...
        public override void ClientDisconnect()
        {
            ...
            if (!NetworkServer.active)
                ...
                {
                    var errorMsg = $"Failed to leave lobby: {error}";
        ...
        public override void ClientConnect(string address)
        {
            ...
            Api.JoinLobby(new LobbyJoinOrLeaveRequest
            {
                ...
                lobby_id = address
            }, () => { StartCoroutine(WaitForLobbyRelay(_lobbyId, false)); }, error =>
            {
                _status = TransportStatus.Offline;
                var errorMsg = $"Failed to join lobby: {error}";
                OnClientError?.Invoke(TransportError.Unexpected, errorMsg);
                Debug.LogError(errorMsg);
        ...
        private IEnumerator WaitForLobbyRelay(string lobbyId, bool forServer)
        {
            ...
            var startTime = NetworkTime.localTime;
            var running = true;
            while (running)
            {
                ...
                {
                    _status = TransportStatus.Error;
                    var errorMsg = "Timed out waiting for lobby.";
                    Debug.LogError(errorMsg);
                    if (forServer)
                    {
                        _status = TransportStatus.Error;
                ...
                Api.GetLobby(lobbyId, lobby =>
                {
                    ...
                    foreach (var aport in lobby.assignment.ports)
                        if (aport.protocol == "UDP")
                        {
                            if (aport.name == "server")
                                relayGameServerPort = (ushort)aport.port;
                            else if (aport.name == "client") relayGameClientPort = (ushort)aport.port;
                        }
                    ...
                    if (!found)
                    {
                        var errorMsg = $"Couldn't find my player ({_playerId})";
                        Debug.LogError(errorMsg);
                ...
                }, error =>
                {
                    ...
                    waitingForResponse = false;
                    _status = TransportStatus.Error;
                    var errorMsg = $"Failed to get lobby info: {error}";
                    Debug.LogError(errorMsg);
                    if (forServer)
                    {
                        OnServerError?.Invoke(0, TransportError.Unexpected, errorMsg);
                ...
                while (waitingForResponse) yield return null;
