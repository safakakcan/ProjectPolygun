...
namespace Edgegap.Editor.Api
{
    ...
    public class EdgegapWizardApi : EdgegapApiBase
    {
        ...
        {
            var json = new JObject { ["source"] = "unity" }.ToString();
            var response = await PostAsync("v1/wizard/init-quick-start", json);
            var result = new EdgegapHttpResult(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        {
            var response = await GetAsync("v1/wizard/registry-credentials");
            var result = new EdgegapHttpResult<GetRegistryCredentialsResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
