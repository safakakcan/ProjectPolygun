...
namespace Edgegap.Editor.Api
{
    ...
    public class EdgegapDeploymentsApi : EdgegapApiBase
    {
        ...
        {
            var response = await PostAsync("v1/deploy", request.ToString());
            var result =
        ...
        ///     It's included in the response of the app deploy. Ex: "93924761ccde"
        /// </param>
        /// <returns>
        ...
        {
            var response = await GetAsync($"v1/status/{requestId}");
            var result =
        ...
        {
            var response = await GetAsync("v1/deployments");
            var result =
        ...
        {
            var response = await DeleteAsync($"v1/stop/{requestId}");
            var result =
        ...
        {
            var createResponse = await CreateDeploymentAsync(
                request
            );

            // Create =>
            var isCreateSuccess = createResponse.StatusCode == HttpStatusCode.OK; // 200
            if (!isCreateSuccess)
                return createResponse;

            // Await Status READY =>
            var requestId = createResponse.Data.RequestId;
