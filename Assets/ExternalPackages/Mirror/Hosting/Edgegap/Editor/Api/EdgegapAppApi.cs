...
namespace Edgegap.Editor.Api
{
    ...
    public class EdgegapAppApi : EdgegapApiBase
    {
        ...
        {
            var result = await UpdateAppVersion(request); // PATCH

            if (result.HasErr)
            {
                // Try to create, instead
                var createAppVersionRequest = CreateAppVersionRequest.FromUpdateRequest(request);
                result = await CreateAppVersion(createAppVersionRequest); // POST
            }

            var isSuccess = result.StatusCode == HttpStatusCode.OK; // 200
        ...
        {
            var response = await PostAsync("v1/app", request.ToString());
            var result = new EdgegapHttpResult<GetCreateAppResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020

            var isSuccess = response.StatusCode == HttpStatusCode.OK; // 200
            if (!isSuccess)
                return result;
        ...
        {
            var response = await GetAsync($"v1/app/{appName}");
            var result = new EdgegapHttpResult<GetCreateAppResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        {
            var relativePath = $"v1/app/{request.AppName}/version/{request.VersionName}";
            var response = await PatchAsync(relativePath, request.ToString());
            var result = new EdgegapHttpResult<UpsertAppVersionResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        {
            var relativePath = $"v1/app/{request.AppName}/version";
            var response = await PostAsync(relativePath, request.ToString());
            var result = new EdgegapHttpResult<UpsertAppVersionResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        {
            var response = await GetAsync("v1/apps");
            var result = new EdgegapHttpResult<GetAppsResult>(response); // MIRROR CHANGE: 'new()' not supported in Unity 2020
        ...
        {
            var response = await GetAsync($"v1/app/{appName}/versions");
            var result = new EdgegapHttpResult<GetAppVersionsResult>(response);
