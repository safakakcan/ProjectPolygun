...
namespace Edgegap
{
    ...
    {
        private static readonly Regex lastDigitsRegex = new("([0-9])+$");
        ...
        {
            if (!File.Exists(path)) throw new Exception("Dockerfile not found, please notify plugin maintainer about this issue.");
            ...
            await RunCommand_DockerVersion(msg => output = msg,
                msg =>
                {
                    if (msg.ToLowerInvariant().Contains("error") || msg.ToLowerInvariant().Contains("invalid")) error = msg;
                });
        ...
        private static async Task RunCommand_DockerPS(Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
#if UNITY_EDITOR_WIN
        ...
        // MIRROR CHANGE
        private static async Task RunCommand_DockerVersion(Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
#if UNITY_EDITOR_WIN
        ...
        public static async Task RunCommand_DockerImage(Action<string> outputReciever, Action<string> errorReciever)
        {
            ...
            await RunCommand("/bin/bash", "-c \"docker image ls --format \"{{.Repository}}:{{.Tag}}\"\"", outputReciever,
                ...
                msg =>
                {
                    if (msg.ToLowerInvariant().Contains("error") || msg.ToLowerInvariant().Contains("invalid")) errorReciever(msg);
                });
        }
        ...
        {
            // ARM -> x86 support:
            var runCommand = IsArmCPU() ? "run --platform linux/amd64" : "run";
            ...
            await RunCommand("/bin/bash", $"-c \"docker {runCommand} --name edgegap-server-test -d {extraParams} {image}\"",
                ...
                msg =>
                {
                    if (msg.ToLowerInvariant().Contains("error") || msg.ToLowerInvariant().Contains("invalid")) throw new Exception(msg);
                });
        }
        ...
        private static async Task RunCommand_InstallLinuxRequirements(string module, string unityVersion, Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
            string error = null;
#if UNITY_EDITOR_WIN
            ...
            if (error != null) errorReciever(error);
        }
        ...
        public static async Task RunCommand_DockerBuild(string dockerfilePath, string registry, string imageRepo, string tag, string projectPath, Action<string> onStatusUpdate, string extraParams = null)
        {
            ...
            var buildCommand = IsArmCPU() ? "buildx build --platform linux/amd64" : "build";

            if (!string.IsNullOrEmpty(extraParams)) buildCommand += $" {extraParams}";
            ...
            await RunCommand("/bin/bash", $"-c \"docker {buildCommand} -f {dockerfilePath} -t {registry}/{imageRepo}:{tag} {projectPath}\"", onStatusUpdate,
                ...
                msg =>
                {
                    if (msg.ToLowerInvariant().Contains("error") || msg.ToLowerInvariant().Contains("invalid")) realErrorMessage = msg;
                    if (msg.ToLowerInvariant().Contains("done")) done = true;
                    Debug.Log(msg);
                    onStatusUpdate(msg);
                });

            if (realErrorMessage != null) throw new Exception(realErrorMessage);

            if (!done) throw new Exception("Couldn't complete containerization, see console log for details.");
        }
        ...
        private static async Task RunCommand(string command, string arguments, Action<string> outputReciever = null, Action<string> errorReciever = null)
        {
            ...
            var customPath = $"{existingPath}:/usr/local/bin";
            ...
            {
                while (!q.IsEmpty)
                    if (q.TryDequeue(out var msg) && !string.IsNullOrWhiteSpace(msg))
                        opt?.Invoke(msg);
            }
        ...
        public static string IncrementTag(string tag)
        {
            ...
            if (!lastDigits.Success) return tag + " _1";

            var number = int.Parse(lastDigits.Groups[0].Value);
        ...
        private static async Task<bool> RunCommand_DockerLogin(
            string registryUrl,
            string repoUsername,
            ...
            Action<string> onStatusUpdate)
        {
            ...
            if (error.ToLowerInvariant().Contains("error") || error.ToLowerInvariant().Contains("invalid")) throw new Exception(error);
            return true;
        }
    }
}
